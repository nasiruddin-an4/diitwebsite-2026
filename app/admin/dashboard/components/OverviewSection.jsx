...
export default function OverviewSection({ data, setActiveSection, adminInfo }) {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-slate-600 text-base mt-2">
          {adminInfo?.role === "notice_admin"
            ? "Welcome, Notice Admin — manage DIIT notices and announcements"
            : "Real-time analytics and content management statistics"}
        </p>
      </motion.div>

      {/* Notice Admin: Simplified View */}
      {adminInfo?.role === "notice_admin" ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {/* Quick Action Card */}
          <div
            onClick={() => setActiveSection("diit-notices")}
            className="group relative bg-white border border-slate-200 rounded-2xl p-8 transition-all duration-300 overflow-hidden cursor-pointer hover:shadow-lg hover:border-blue-200"
          >
            <div className="absolute top-0 right-0 w-48 h-48 bg-linear-to-br from-blue-100 to-transparent opacity-50 -mr-16 -mt-16 rounded-full group-hover:scale-150 transition-transform duration-500" />
            <div className="relative z-10 flex items-center gap-6">
              <div className="w-16 h-16 rounded-xl bg-linear-to-br from-blue-600 to-indigo-700 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg shadow-blue-600/20">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Create, edit, and manage official notices and announcements
                </p>
              </div>
              <ArrowUpRight className="w-6 h-6 text-blue-500 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </div>

          {/* Info Card */}
          <div className="bg-linear-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white shadow-xl">
            <h3 className="text-sm font-bold opacity-60 uppercase tracking-widest mb-4">
              Your Permissions
            </h3>
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span className="text-sm font-medium">Delete notices</span>
              </div>
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span className="text-sm font-medium">
                  Upload images and PDFs to notices
                </span>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="flex items-center justify-between mb-8">
                      <div
                        className={`w-10 h-10 rounded-md bg-linear-to-br ${stat.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                      >
                        <stat.icon className="w-5 h-5 text-white" />
                      </div>
                      <span
                        className={`px-2 py-1 rounded-md text-[12px] font-bold tracking-wider ${
                          stat.status.includes("Pending")
                            ? "bg-amber-100 text-amber-700"
                            : stat.status.includes("Active")
                    ...
                    <div>
                      <p className="text-slate-600 text-[16px] font-bold mb-4">
                        {stat.label}
                      </p>
                      <p className="text-2xl font-black text-slate-900 leading-none">
                        {loadingAdmissions && stat.id === "online"
                          ? "..."
                          : stat.value}
                      </p>
                    </div>
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="bg-linear-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white shadow-xl shadow-slate-200">
                <h3 className="text-sm font-bold opacity-60 uppercase tracking-widest mb-6">
                  Database Health
                </h3>
                <div className="space-y-4">
                  {[
                    {
                      label: "News Records",
                      count: data?.newsEvents?.length || 0,
                      max: 50,
                      color: "bg-purple-500",
                    },
                    {
                      label: "MoU Partners",
                      count: data?.internationalPartners?.length || 0,
                      max: 50,
                      color: "bg-blue-500",
                    },
                    {
                      label: "Hero Content",
                      count: data?.heroSlides?.length || 0,
                      max: 20,
                      color: "bg-rose-500",
                    },
                  ].map((item, i) => (
