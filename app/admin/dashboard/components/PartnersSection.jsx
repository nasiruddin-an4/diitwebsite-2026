...
import { Plus, ChevronRight, Users, Edit2, Trash2, Save, Loader2 } from "lucide-react";
import { InputField } from "./InputField";

export default function PartnersSection({ data, updateField, addItem, deleteItem, onSave, saving }) {
  const [editingId, setEditingId] = useState(null);
  const [deleteId, setDeleteId] = useState(null);
  const template = { id: "", name: "", logo: "", country: "", type: "", description: "" };

  const handleAdd = () => {
    const newId = Date.now();
    addItem("internationalPartners", { ...template, id: newId });
    setEditingId(newId);
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between text-slate-900 border-b border-slate-200 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold">Edit Partner</h2>
          </div>
          <button
            onClick={() => setEditingId(null)}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <InputField label="Type" value={editingPartner.type} onChange={(v) => updateField("internationalPartners", editingIndex, "type", v)} />
            <InputField label="Logo URL" value={editingPartner.logo} onChange={(v) => updateField("internationalPartners", editingIndex, "logo", v)} />
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Partner Name</th>
              ...
              {partners.length === 0 ? (
                ...
                partners.map((partner, index) => (
                  <tr key={partner.id || index} className="group hover:bg-slate-50 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0 p-1.5">
                          {partner.logo ? (
                    ...
                    <td className="p-4 text-slate-500 text-xs">
                      <span className="px-2 py-1 bg-slate-100 rounded text-xs border border-slate-200 text-slate-600">
                        {partner.type || "N/A"}
                      </span>
                    </td>
                    <td className="p-4">
                      <div className="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          onClick={() => setDeleteId(partner.id || partner._id)}
        ...
        {deleteId && (
          ...
          >
            <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">Delete Partner?</h3>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={() => {
                    const index = partners.findIndex(p => (p.id || p._id) === deleteId);
                    if (index !== -1) deleteItem("internationalPartners", index);
                    setDeleteId(null);
                  }}
