...
export default function NewsSection({ data, updateField }) {
  ...
  const handleSaveItem = async () => {
    try {
      ...
      let allItems = Array.isArray(data?.newsEvents) ? data.newsEvents : [];
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const updatedList = (
            Array.isArray(data?.newsEvents) ? data.newsEvents : []
          ).filter((i) => (i._id || i.id) !== targetId);
          updateField("newsEvents", null, null, updatedList);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Manage latest news, events and announcements
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/50">
                <th className="w-[45%] p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Article Details
                </th>
                ...
                newsItems.map((item, index) => (
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-slate-500 text-xs truncate mt-1 leading-relaxed">
                            {item.excerpt || item.desc || "No summary provided"}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4 text-center">
                      <span
                        className={`inline-block px-3 py-1 rounded-md text-[10px] font-semibold tracking-wider
                            ${
                              item.category === "EVENT"
                    ...
                    <td className="p-4 text-center">
                      <div className="flex flex-col items-center justify-center">
                        <p className="text-slate-700 text-xs font-bold">
                          {item.date?.split(",")[0] || item.date || "TBD"}
                        </p>
                      </div>
                    </td>
                    <td className="p-4">
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                  ...
                  <div className="md:col-span-4 space-y-4">
                    <div className="space-y-4">
                      ...
                      {editingItem.image && (
                        <button
                          onClick={() => updateLocalField("image", "")}
                          className="block w-full text-center text-[10px] font-bold text-red-500 uppercase hover:text-red-600"
                        >
                          Remove Image
                        </button>
                      )}
              ...
              <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 rounded-b-3xl">
                ...
                <button
                  ...
                  disabled={processing}
                  className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-bold transition-all text-sm cursor-pointer flex items-center gap-2"
                >
