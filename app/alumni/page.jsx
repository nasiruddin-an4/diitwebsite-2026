...
const AlumniPage = () => {
  ...
  const filteredAlumni = alumni.filter((alumnus) => {
    ...
    const matchesDept =
      selectedDept === "All" || alumnus.department === selectedDept;
  ...
  const paginatedItems = filteredAlumni.slice(
    startIndex,
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans selection:bg-brandColor/10 selection:text-brandColor overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed font-medium">
              Meet the professionals who are shaping the future and leading
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-10 relative z-20 pb-32">
        ...
        <motion.div
          ...
          className={`bg-white border border-slate-100 rounded-2xl p-3 mb-10 shadow-[0_20px_50px_rgba(0,0,0,0.03)] sticky top-20 z-40 transition-all duration-500 ${isScrolled ? "mx-4 lg:mx-0 shadow-[0_25px_60px_rgba(0,0,0,0.08)]" : ""}`}
        >
              ...
              {paginatedItems.map((alumnus, index) => (
                ...
                >
                  ...
                  <div className="relative aspect-[1/1.1] overflow-hidden mb-4 bg-slate-100 rounded-sm">
                    <Link
                      href={`/alumni/${alumnus._id || alumnus.id}`}
                    ...
                    <div className="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                      {alumnus.linkedin && (
                        <a
                          ...
                          className="w-10 h-10 bg-white text-brandColor flex items-center justify-center shadow-xl hover:bg-brandColor hover:text-white transition-colors"
                        >
          ...
          <div className="mt-20 flex flex-col md:flex-row items-center justify-between gap-8 bg-white/50 backdrop-blur-sm p-6 rounded-[2rem] border border-white">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-12 h-12 rounded-2xl bg-white shadow-sm text-slate-600 flex items-center justify-center hover:bg-brandColor hover:text-white disabled:opacity-20 transition-all cursor-pointer"
              >
              ...
              <div className="flex items-center gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-12 h-12 rounded-2xl text-sm font-bold transition-all ${currentPage === i + 1 ? "bg-brandColor text-white shadow-lg" : "text-slate-400 bg-white hover:bg-slate-50"}`}
                  >
                    {i + 1}
                  </button>
                ))}
